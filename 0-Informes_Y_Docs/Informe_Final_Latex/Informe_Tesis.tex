...
\begin{enumerate}
\item Eliminamos filas repetidas e innecesarias.
\item Unimos los datasets.
\item Limpieza inicial y normalización.
\begin{itemize}
	\item Codificamos nuestros textos a utf-8.
    \item Convertimos todo a minúscula.
    \item Eliminamos datos no relevantes para nuestros análisis (mails, páginas web y common words).
    \item Eliminamos signos de puntuación y caracteres especiales (incluyendo números).
    \item Eliminamos stop words.
\end{itemize}
\item Aplicamos Tokenización.
\item Aplicamos Lematización.
\item Obtenemos y usamos bi-gramas.
\end{enumerate}

Si bien en la sección Ver \textit{\nameref{Proces_textos}} se detalla en qué consiste resumidamente cada etapa, a continuación realizaremos un mayor acercamiento con ejemplos visuales de cada una de las mismas.

En el primer paso, \textit{1. Eliminamos filas repetidas e innecesarias}, eliminamos los duplicados (\textit{filas repetidas}) de cada uno de los datasets. Además, en algunos casos se tuvieron que eliminar \textit{filas innecesarias}. ¿A qué llamamos \textit{filas innecesarias}?: Por ejemplo, para el caso del dataset visualizado en la Figura \ref{fig:DS_Cand_3}, además de trabajos de IT, también este dataset cuenta con candidatos que pertenecen a trabajos de arte, ventas, salud y deporte, ingeniería eléctrica, civil o mecánica, etc. Para nuestro análisis únicamente nos interesan los trabajos relacionados a IT, es por esto que filtramos únicamente las filas con curriculums que pertenezcan a trabajos de IT (los cuales eran, por ejemplo, los pertenecientes a las categorías 'Data Science', 'Web Designing', 'Java Developer', etc.).
...
Luego, en el paso \textit{2. Unimos los datasets}, lo que se hizo fue unir por un lado los 4 datasets de candidatos, y por el otro los 2 datasets de puestos de IT. De esta manera, en la Figura \ref{fig:final_antes_preproc} podemos observar los tamaños finales de nuestros datasets y sus repectivas columnas. Este paso fue necesario para simplificar la futura aplicación de las respectivas funciones de limpieza de datos.
...
El siguiente paso, \textit{3.Limpieza inicial y normalización}, consiste en limpiar y normalizar nuestros datos, tanto de los curriculums vitae como de las descripciones de los puestos de IT. Para esto transformamos nuestros textos codificándolos a utf-8, convertimos todos los caracteres a minúscula y eliminamos datos no relevantes para nuestros análisis (mails, páginas web y common words), signos de puntuación, caracteres especiales (incluyendo números) y stop words.
...
Podemos observar que debido al paso \textit{3.Limpieza inicial y normalización} pasaron a minúsculas todas las palabras, se eliminaron los signos de puntuación y carácteres especiales (paréntesis, comas, puntos, números, etc.), stop words ('he', 'has', 'in', 'and', etc.), common words (las vistas en la Figura \ref{fig:common_text}), nombre de los candidatos (en este caso 'Calonge Federico Matias'), y páginas web ('https://github.com/FedericoCalonge/')\footnote{Las páginas webs y correos se eliminaron mediante expresiones regulares.}.

Tambien podemos visualizar algunos lemmas que se obtuvieron, notando mayormente en este caso que las palabras en plural pasaron a su forma/lema singular: 'tools' a 'tool', 'projects' a 'project', etc.

Por último, en la etapa \textit{6.Obtenemos y usamos bi-gramas}, se decidió asociar 2 palabras que ocurren frecuentemente como 1 sola palabra utilizando bi-grams (como por ejemplo \detokenize{machine_learning}, \detokenize{data_science}, etc.) con el objetivo de enriquecer nuestro vocabulario. Esto nuevamente se aplicó en los textos de ambos datasets. Siguiendo con el ejemplo de la Figura \ref{fig:tokens_obt}, al aplicar bi-grams obtuvimos los tokens visualizados en la Figura \ref{fig:bigrams_obt}.
...
Una vez habiendo realizado la etapa de preprocesamiento y limpieza de datos nos quedaron los siguientes tamaños de nuestros datasets:
\begin{itemize}
...
\end{itemize}
Nota: No obstante, al realizar los cálculos de distancias compararemos cada curriculum vitae con cada Job Description, obteniendo un dataframe total de 3131 filas con sus respectivo valores de WMD y Cosine Similarity.
