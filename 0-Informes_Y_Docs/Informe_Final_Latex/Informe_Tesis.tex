...
Como se mencionó en la sección \textit{\nameref{etapas_set_datos}}, utilizaremos los 624 Curriculum Vitae de candidatos y 20593 descripciones de puestos de IT para el entrenamiento y obtención de vectores mediante TF-IDF (\textbf{21217} filas en total).
...
Como se mencionó en la sección \textit{\nameref{etapas_set_datos}}, utilizaremos los 624 Curriculum Vitae de candidatos y 20593 descripciones de puestos de IT para el entrenamiento y obtención de word embeddings mediante Word2vec (\textbf{21217} filas en total).
...
De esta forma se puede calcular el WMD para esta candidata y descripción del puesto utilizando estos vectores y los word embeddings de sus palabras\footnote{El cálculo de WMD que se realiza entre 2 vectores fue descrito en la sección \textit{\nameref{wmd}}.}, dándonos como resultado una medida de similitud entre los mismos: visto previamente en la Imagen \ref{fig:7_Calculo_WMD}.
...
Sin embargo, debido a los problemas que puedan llegar a ocurrir y que detallamos en la sección \textit{\nameref{implementar_mod_ML} (Paso 6: Parameter Tuning o configuración de parámetros)}, se optó por utilizar cross validation para realizar la validación del modelo iterando sobre los k hiper-parámetros para llegar al k más óptimo, método descrito en la sección \textit{\nameref{Cross Validation}}.
...
Además de la mejora propuesta relacionada a fase de validación, como vimos en la sección \textit{\nameref{limitacionessistema}}, nuestro sistema cuenta con algunas limitaciones adicionales para las cuales se podrían implementar una serie de mejoras, mencionadas a continuación. 
